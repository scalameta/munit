...
class SkippedFrameworkSuite extends FunSuite {
  ...
  test("ignore".ignore)(???)
  test("ignore.failed.pending".ignore.pending)(assert(false))
  test("ignore.failed.pending.comment".ignore.pending("comment"))(assert(false))
  test("assume(true)")(
    assume(true, "assume it passes")
    // println("pass")
  )
  test("assume(false)")(assume(false, "assume it fails"))
  ...
  test("pending.successful.ignored".pending)(assert(true))
  test("pending.successful.ignored.comment".pending("comment"))(assert(true))
  test("pending.failed.not-ignored".pending)(assert(false))
  test("pending.failed.not-ignored.comment".pending("comment"))(assert(false))
  test("pending.failed.ignored".pending.ignore)(assert(false))
  test("pending.failed.ignored.comment".pending("comment").ignore)(assert(false))
}
...
object SkippedFrameworkSuite
    extends FrameworkTest(
      ...
      """|==> success munit.SkippedFrameworkSuite.pass
         ...
         |==> failure munit.SkippedFrameworkSuite.pending.failed.not-ignored - tests/shared/src/main/scala/munit/SkippedFrameworkSuite.scala:25 assertion failed
         ...
         |==> failure munit.SkippedFrameworkSuite.pending.failed.not-ignored.comment - tests/shared/src/main/scala/munit/SkippedFrameworkSuite.scala:26 assertion failed
         |25:  test("pending.failed.not-ignored".pending)(assert(false))
         |26:  test("pending.failed.not-ignored.comment".pending("comment"))(assert(false))
