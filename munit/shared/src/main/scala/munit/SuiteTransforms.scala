...
trait SuiteTransforms {
  ...
  def munitSuiteTransforms: List[SuiteTransform] =
    List(munitIgnoreSuiteTransform, munitOnlySuiteTransform)

  final def munitSuiteTransform(tests: List[Test]): List[Test] =
    try munitSuiteTransforms.foldLeft(tests) { case (ts, fn) => fn(ts) }
    catch {
      case NonFatal(e) => List(
          new Test("munitSuiteTransform", () => Future.failed(e))(Location.empty)
        )
    }

  def munitIgnore: Boolean = false
  final def munitIgnoreSuiteTransform: SuiteTransform =
    new SuiteTransform("munitIgnore", { tests => if (munitIgnore) Nil else tests })

  def isCI: Boolean = "true" == System.getenv("CI")
  final def munitOnlySuiteTransform: SuiteTransform = new SuiteTransform(
    "only",
    { tests =>
