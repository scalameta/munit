...
final class Test(
  ...
  def withName(newName: String): Test = copy(name = newName)
  def withBody(newBody: () => Future[Any]): Test = copy(body = newBody)
  def withTags(newTags: Set[Tag]): Test = copy(tags = newTags)
  def tag(newTag: Tag): Test = withTags(tags + newTag)
  def withLocation(newLocation: Location): Test = copy(location = newLocation)
  ...
  ): Test = new Test(name, body, tags, location)
