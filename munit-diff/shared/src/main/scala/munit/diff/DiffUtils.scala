...
object DiffUtils {
  def generateUnifiedDiff(
    ...
    if (!patch.getDeltas.isEmpty) {
      ...
      if (patchDeltas.size() > 1) for (i <- 1 until patchDeltas.size) {
  ...
  ): Patch[String] = new MyersDiff[String]().diff(original, revised)
  ...
  private def processDeltas(
    ...
    while (deltaIndex < deltas.size) { // for each of the other Deltas
      ...
      val intermediateStart = curDelta.getOriginal.getPosition +
        curDelta.getOriginal.getLines.size
      line = intermediateStart
      while (line < nextDelta.getOriginal.getPosition) { // output the code between the last Delta and this one
        buffer.add(" " + origLines.get(line))
        origTotal += 1
        revTotal += 1
  ...
  private def getDeltaText(delta: Delta[String]) = {
    ...
    val buffer = new util.ArrayList[String]
    for (line <- delta.getOriginal.getLines.asScala) buffer.add("-" + line)
    for (line <- delta.getRevised.getLines.asScala) buffer.add("+" + line)
    buffer
  }
